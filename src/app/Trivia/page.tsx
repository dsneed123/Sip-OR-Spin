...
export default function TriviaGame() {
  ...
  useEffect(() => {
    if (showResult) {
      ...
      localStorage.setItem("triviaResult", JSON.stringify(passed)); // Store result in local storage
      setTimeout(() => {
      ...
      <Card className="max-w-md w-full p-6 text-center shadow-lg bg-white rounded-xl">
        {showResult ? (
          <CardContent>
            ...
            <p className={`mt-2 text-lg font-semibold ${score >= 3 ? "text-green-600" : "text-red-600"}`}>
              {score >= 3 ? "You passed!" : "You failed!"}
            </p>
            <p className="mt-2 text-sm">Returning in 5 seconds...</p>
          </CardContent>
        ) : (
